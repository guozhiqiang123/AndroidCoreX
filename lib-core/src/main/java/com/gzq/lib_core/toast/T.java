package com.gzq.lib_core.toast;
...
import com.gzq.lib_core.toast.style.ToastBlackStyle;
import com.gzq.lib_core.toast.style.ToastQQStyle;
import com.gzq.lib_core.toast.style.ToastWhiteStyle;
...
public final class T {
    ...
    public void init(Application application) {
        init(application, new ToastBlackStyle());
    }
     ...
     * @param yOffset y轴偏移
     */
    public T setGravity(int gravity, int xOffset, int yOffset) {
        checkToastState();
     ...
     */
    public T initStyle(IToastStyle style) {
        T.sDefaultStyle = style;
        // 如果吐司已经创建，就重新初始化吐司
        if (sToast != null) {
            // 取消原有吐司的显示
            sToast.cancel();
    ...
    /**
     * 检查吐司状态，如果未初始化请先调用{@link T#init(Application)}
     */
    private static void checkToastState() {
        // 吐司工具类还没有被初始化，必须要先调用init方法进行初始化
        if (sToast == null) {
            throw new IllegalStateException("T has not been initialized");
        }
    }
